...
function placeXORo(squareNumber) {
    ...
    if (!selectedSquares.some(element => element.includes(squareNumber))) {
        ...
        if (activePlayer === 'X') {
            ...
            //Active played may only be 'X' or 'O' so, if not 'X' it must be 'O'
        } else {
...
function checkWinConditions() {
    ...
    //This condtion checks for a tie. If none of the above conditions are met and 
    //9 squares are selected the code executes.
    else if (selectedSquares.length >= 9) {
        //This function plays the tie game sound.
        audio('./media/tie.mp3');
        //This functions sets a .3 second time before the resetGame is called.
...
function drawWinLine(coordX1, coordY1, coordX2, coordY2) {
    ...
    let x1 = coordX1,
        ...
        y2 = coordY2,
        //This variable stores the temporary x axis data we update in our animation loop.
        x = x1,
        //This variable stores the temporary y axis data we update in our animation loop.
        y = y1;
        ...
        function animateLineDrawing() {
            ...
            //This condition is similar to the one above.
            //This is also necessary for the 6, 4, 2 win condition.
            if (x1 <= x2 && y1 >= y2) {
                if (x < x2) { x += 10; }
                if (y > y2) { y -= 10; }
                if (x >= x2 && y <= y2) { cancelAnimationFrame(animationLoop); }
